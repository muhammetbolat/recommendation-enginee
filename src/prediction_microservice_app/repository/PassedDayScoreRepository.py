...
class PassedDayScoreRepository(Generic[T]):
    @inject
    ...
    def get_by_products(self, products: list):
        result: list = []
        query = self.database_session_manager.session.query(self.type)
        for productId in products:
            product_score = query.filter_by(productId=productId).first()
            if product_score is not None:
                result.append(product_score.score)
            else:
                result.append(0)
        return result
    ...
    def save(self, entity: T):
        ...
        if entity_exist is not None:
            entity_exist.score = entity.score
        else:
            self.insert(entity)
