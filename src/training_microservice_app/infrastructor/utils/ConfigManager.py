...
from model.configs.BaseConfig import BaseConfig
...
class ConfigManager:
    ...
    def __get_configs(root_directory) -> []:
        config_path = os.path.join(root_directory, "model", "configs")
        ...
        if environment is not None:
            config_path_splited = "application.yml".split('.')
            config_path = f'{config_path_splited[0]}.{environment}.{config_path_splited[1]}'
